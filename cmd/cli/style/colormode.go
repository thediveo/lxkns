...
func ColorModeSetupCLI(rootCmd *cobra.Command) {
	...
	rootCmd.PersistentFlags().VarP(
		enumflag.New(&colorize, ColorFlagName, colorModeIds, enumflag.EnumCaseSensitive),
		ColorFlagName, ColorFlagShorthand,
...
func ColorModeBeforeCommand(cmd *cobra.Command) error {
	colorize := cmd.PersistentFlags().Lookup(ColorFlagName).
		Value.(*enumflag.EnumFlagValue[ColorMode]).GetValue()
	// Colorization mode...
	switch colorize {
	case ColorAlways:
