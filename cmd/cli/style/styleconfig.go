...
func parseStyles(w io.Writer, configyaml string) {
	yml := map[string]any{}
	...
	for elementKey, elementStyles := range yml {
		...
		// "attribute" elements.
		for _, elementStyle := range elementStyles.([]any) {
			parseElementStyle(w, sty, elementStyle)
		}
	}
}
...
func parseElementStyle(w io.Writer, sty *Style, elementStyle any) {
	...
	if attr, ok := elementStyle.(string); ok {
		...
		// API.
		if colormap, ok := elementStyle.(map[string]any); ok {
			...
			_, _ = fmt.Fprintf(w, "invalid color mapping item %v", elementStyle)
		}
	}
}
