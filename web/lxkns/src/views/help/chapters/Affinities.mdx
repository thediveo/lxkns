...
import OffCPUIcon from 'icons/OffCPU';
...
All currently online CPUs are listed at the outmost indentation level, with an
<CPUIcon className="icon" /> icon and their logical CPU number.
...
The per-CPU trees show only processes and tasks that are allowed to run on this
particular CPU; the exception are processes that aren't allowed but yet have one
or more child processes that are allowed on this CPU: such intermediate
"non-allowed" processes are marked with <OffCPUIcon className="icon" /> as well
as completely grayed out. 
...
- <Pinned className="icon" /> signals that a task or process can **execute
  only** on this particular CPU, but not on any other CPU.

- <RealtimeIcon className="icon" /> points out that there is a realtime process
  or task deeper down the hierarchy and it is scheduled with one of these
  policies: FIFO, RR, or DEADLINE.
...
- <OffCPUIcon className="icon" /> and **grayed-out** task or process information
  signals that a process **can't execute** on this CPU but is included as a
  result of the process hierarchy.
