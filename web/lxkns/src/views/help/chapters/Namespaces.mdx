...
Lxkns follows the textual representation of namespace identifiers as used in the
Linux kernel, for instance, when executing "`ls -l /proc/self/ns/net`": the type
of namespace, followed by the inode number of the namespace. The identifier of a
namespace is constant only during its lifetime and can be recycled at any time
afterwards.
...
Certain system operations can be carried out only in initial namespaces and are
blocked in other non-initial namespaces.
