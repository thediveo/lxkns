...
For identifying individual namespaces, **lxkns** follows the textual
representation used throughout the Linux kernel, for instance, when executing
"`ls -l /proc/self/ns/net`": the type of namespace, followed by the inode number
of the namespace. The identifier of a namespace is constant only during its
lifetime and can be recycled at any time afterwards.
