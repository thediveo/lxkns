/*
Package discover discovers [Linux kernel namespaces] of types cgroup, ipc,
mount, net, pid, time, user, and uts. This package discovers namespaces not only
when processes have joined them, but also when namespaces have been bind-mounted
or are only referenced anymore by process file descriptors or within a hierarchy
(PID and user only).

Please note that our manual has some nice [UML diagrams depicting Linux'
namespaces model from 10,000m].
...
basic level comprises ... namespaces. Again, the manual has some nice [namespace
representation UML diagram].

In the previous code snippet, the information model returned is stored in the
“allns” variable for further processing. The result organizes the namespaces
found by type. For instance, the following code snippet prints all namespaces,
sorted first by type and then by namespace identifier:
...
This parent-child hierarchy is exposed through the [model.Hierarchy] interface
of the discovered namespaces.
...
user-type namespaces a [model.Namespace] can be “converted” into an interface
...
And non-user namespaces are owned by user namespaces, the “ownings”. In case
you are now scratching your head “why the Gopher” the owned namespaces are
referred to as “ownings”: welcome to the wonderful Gopher world of “er”-ers,
where interface method naming conventions create wonderful identifier art.

If a namespace interface value represents a user-type namespace, then it can be
“converted” into an [model.Ownership] interface value using a type assertion.
This interface discloses which namespaces are owned by a particular user
namespace. Please note that this does not include child user namespaces, use
...
section of the lxkns manual].

[architectural section of the lxkns manual]: https://thediveo.github.io/lxkns/#/model
[Linux kernel namespaces]: https://man7.org/linux/man-pages/man7/namespaces.7.html
[Discovering Namespaces UML diagrams]: https://thediveo.github.io/lxkns/#/model?id=discovering-namespaces
[UML diagrams depicting Linux' namespaces model from 10,000m]: https://thediveo.github.io/lxkns/#/model?id=linux-namespaces-from-10000m
[namespace representation UML diagram]: https://thediveo.github.io/lxkns/#/model?id=linux-namespace-representation-in-lxkns
